...
const useAlarmListeners = () => {
    useEffect(() => {
        ...
        const doneSubscription = DeviceEventEmitter.addListener('alarmDone', (event) => {
        ...
        const snoozeSubscription = DeviceEventEmitter.addListener('alarmSnooze', (event) => {
...
function RootLayoutNav() {
    ...
    return (_jsxs(Stack, { screenOptions: { ...(isIOS ? { headerBackTitle: "Back" } : {}) }, children: [_jsx(Stack.Screen, { name: "index", options: { headerShown: false } }), _jsx(Stack.Screen, { name: "settings", options: { headerShown: false } }), _jsx(Stack.Screen, { name: "alarm", options: {
                    presentation: "fullScreenModal",
                    headerShown: false,
                    gestureEnabled: false,
                } }), _jsx(Stack.Screen, { name: "notifications-debug", options: { title: 'Notifications Debug' } })] }));
}
function AppContent() {
        ...
        (async () => {
            try {
                ...
                if (initial?.notification) {
                    const reminderId = initial.notification.data?.reminderId;
                    const priority = initial.notification.data?.priority;
                ...
                if (type === EventType.ACTION_PRESS && notification && pressAction) {
                    ...
                    if (pressAction.id === 'done') {
                        ...
                        const doneTimestamp = triggerTime
                            ? (typeof triggerTime === 'number' ? triggerTime : parseInt(triggerTime, 10))
                            : undefined;
            ...
            try {
                ...
                if (settingsStr) {
                    ...
                    if (AlarmModule?.saveNotificationSettings) {
                        await AlarmModule.saveNotificationSettings(settings.soundEnabled ?? true, settings.vibrationEnabled ?? true);
                        // Sync ringer volume
    ...
    if (isLoading) {
        return _jsx(View, { style: { flex: 1 } });
    }
    return (_jsx(ThemeProvider, { children: _jsx(ErrorBoundary, { children: _jsx(GestureHandlerRootView, { style: styles.root, onLayout: onLayoutRootView, children: _jsxs(ReminderEngineProvider, { children: [_jsx(StatusBar, { style: "dark", backgroundColor: "transparent", translucent: true }), _jsx(RootLayoutNav, {})] }) }) }) }));
}
export default function RootLayout() {
    return (_jsx(QueryClientProvider, { client: rootQueryClient, children: _jsx(AppContent, {}) }));
}
