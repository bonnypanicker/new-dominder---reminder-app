...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, Pressable, Alert, Modal, TextInput, Dimensions, Keyboard as RNKeyboard, Platform, PanResponder, KeyboardAvoidingView, Animated, FlatList } from 'react-native';
...
const Plus = (props) => _jsx(Feather, { name: "plus", ...props });
const Clock = (props) => _jsx(Feather, { name: "clock", ...props });
const Settings = (props) => _jsx(Feather, { name: "settings", ...props });
const PauseCircle = (props) => _jsx(Feather, { name: "pause-circle", ...props });
const PlayCircle = (props) => _jsx(Feather, { name: "play-circle", ...props });
const CheckCircle = (props) => _jsx(Feather, { name: "check-circle", ...props });
const Trash2 = (props) => _jsx(Feather, { name: "trash-2", ...props });
const RotateCcw = (props) => _jsx(Feather, { name: "rotate-ccw", ...props });
const AlertCircle = (props) => _jsx(Feather, { name: "alert-circle", ...props });
const X = (props) => _jsx(Feather, { name: "x", ...props });
const Square = (props) => _jsx(Feather, { name: "square", ...props });
const CheckSquare = (props) => _jsx(Feather, { name: "check-square", ...props });
const Repeat = (props) => _jsx(Feather, { name: "repeat", ...props });
const HelpCircle = (props) => _jsx(Feather, { name: "help-circle", ...props });
const Keyboard = (props) => _jsx(MaterialIcons, { name: "keyboard", ...props });
...
const debouncedUpdate = (callback, delay = 50) => {
    if (updateTimeoutId) {
...
export default function HomeScreen() {
    ...
    const handleTabLayout = useCallback((tab, event) => {
        ...
        setTabLayouts(prev => {
            if (prev[tab]?.x === x && prev[tab]?.width === width)
                return prev;
    ...
    const [untilIsAM, setUntilIsAM] = useState(() => {
        const defaultTime = calculateDefaultTime();
        return defaultTime.isAM;
    });
    const [showTimeSelector, setShowTimeSelector] = useState(false);
    // Track which time we are editing in the shared TimeSelector
    const [timeSelectorContext, setTimeSelectorContext] = useState('start');
    const [title, setTitle] = useState('');
    const [priority, setPriority] = useState('medium');
    const [repeatType, setRepeatType] = useState('none');
    const [repeatDays, setRepeatDays] = useState([]);
    const [everyValue, setEveryValue] = useState(1);
    const [everyUnit, setEveryUnit] = useState('hours');
    ...
    const [untilType, setUntilType] = useState('none');
    const [untilDate, setUntilDate] = useState('');
    const [untilCount, setUntilCount] = useState(1);
    const [editingReminder, setEditingReminder] = useState(null);
    const [pauseUntilCalendarVisible, setPauseUntilCalendarVisible] = useState(false);
    const [pauseUntilReminder, setPauseUntilReminder] = useState(null);
    ...
    const isSelectionModeRef = React.useRef(false);
    ...
    const [selectionTimestamp, setSelectionTimestamp] = React.useState(null);
    ...
    const completeReminder = useCallback((reminder) => {
    ...
    const completeAllOccurrences = useCallback((reminder) => {
    ...
    const pauseReminder = useCallback((reminder) => {
    ...
    const handlePauseUntilDate = useCallback((date) => {
        if (!pauseUntilReminder)
            return;
    ...
    const reassignReminder = useCallback((reminder) => {
        ...
        if (reminder.repeatType === 'none') {
            if (reminderDateTime <= now) {
                Alert.alert('Past Time', 'This reminder\'s time has already passed. It cannot be reassigned to active reminders.', [{ text: 'OK' }]);
                return;
            }
        }
        ...
        if (reminder.repeatType === 'daily') {
            ...
            if (todayTime <= now) {
                Alert.alert('Note', 'The time has already passed for today. This reminder will be active starting tomorrow.', [{
                        text: 'OK', onPress: () => {
                            updateReminder.mutate({
                                ...reminder,
                                isCompleted: false,
    ...
    const to12h = useCallback((time24) => {
    ...
    const handlePermanentDelete = useCallback((reminder) => {
        // Mark deletion in progress to prevent extraData updates that would interrupt animation
        isDeletingRef.current = true;
        setTimeout(() => {
            isDeletingRef.current = false;
    ...
    const handleRestore = useCallback((reminder) => {
        restoreReminder.mutate(reminder.id);
    }, [restoreReminder]);
    const handleLongPress = useCallback((reminderId, tab) => {
    ...
    const handleCardPress = useCallback((reminder) => {
    ...
    const formatRepeatType = useCallback((repeatType, everyInterval) => {
        switch (repeatType) {
    ...
    const ReminderCard = memo(({ reminder, listType, isSelected, isSelectionMode: selectionMode }) => {
        ...
        if (isCompletedOrDeleted) {
            return (_jsx(SwipeableRow, { reminder: reminder, swipeableRefs: swipeableRefs, simultaneousHandlers: contentScrollRef, onSwipeRight: !selectionMode ? () => handlePermanentDelete(reminder) : undefined, onSwipeLeft: !selectionMode ? () => handlePermanentDelete(reminder) : undefined, isSelectionMode: selectionMode, leftActionType: "delete", children: _jsx(TouchableOpacity, { activeOpacity: 0.85, onPress: () => handleCardPress(reminder), onLongPress: () => handleLongPress(reminder.id, listType), delayLongPress: 200, style: [
                        styles.reminderCardCompact,
                        isSelected && styles.selectedCard
                    ], testID: `reminder-card-${reminder.id}`, children: _jsxs(View, { style: styles.reminderContentCompact, children: [_jsxs(View, { style: styles.reminderLeftCompact, children: [selectionMode && (_jsx(TouchableOpacity, { style: styles.selectionCheckbox, onPress: () => handleCardPress(reminder), children: isSelected ? (_jsx(CheckSquare, { size: 20, color: Material3Colors.light.primary })) : (_jsx(Square, { size: 20, color: Material3Colors.light.onSurfaceVariant })) })), _jsx(View, { style: [styles.priorityBarCompact, { backgroundColor: PRIORITY_COLORS[reminder.priority] }] }), _jsx(Text, { style: styles.reminderTitleCompact, numberOfLines: 1, ellipsizeMode: "tail", children: reminder.title }), _jsx(Text, { style: styles.compactSeparator, children: "\u2022" }), _jsx(Text, { style: styles.reminderTimeCompact, children: (() => {
                                            ...
                                            return formatTime(reminder.time);
                                        })() }), _jsxs(_Fragment, { children: [_jsx(Text, { style: styles.compactSeparator, children: "\u2022" }), _jsx(Text, { style: styles.reminderDateCompact, numberOfLines: 1, children: (() => {
                                                ...
                                                })() })] }), reminder.repeatType && (_jsxs(_Fragment, { children: [_jsx(Text, { style: styles.compactSeparator, children: "\u2022" }), _jsx(View, { style: [styles.repeatBadge, { paddingVertical: 1, paddingHorizontal: 6, minHeight: 0 }], children: _jsx(Text, { style: [styles.repeatBadgeText, { fontSize: 11 }], children: formatRepeatType(reminder.repeatType, reminder.everyInterval) }) })] })), reminder.repeatType === 'every' && reminder.everyInterval && (_jsxs(_Fragment, { children: [_jsx(Text, { style: styles.compactSeparator, children: "\u2022" }), _jsxs(Text, { style: { fontSize: 11, color: Material3Colors.light.onSurfaceVariant, fontWeight: '600' }, children: [reminder.everyInterval.value, reminder.everyInterval.unit === 'minutes' ? 'm' : reminder.everyInterval.unit === 'hours' ? 'h' : 'd'] })] }))] }), (reminder.completionHistory && reminder.completionHistory.length > 0) && (_jsx(TouchableOpacity, { style: [
                                    styles.historyBadge,
                                    ...
                                    ], children: reminder.completionHistory.length }) }))] }) }) }));
        }
        // Full layout for active reminders
        return (_jsx(SwipeableRow, { reminder: reminder, swipeableRefs: swipeableRefs, simultaneousHandlers: contentScrollRef, onSwipeRight: !selectionMode ? (isDeleted ? () => handlePermanentDelete(reminder) : () => handleDelete(reminder)) : undefined, onSwipeLeft: !selectionMode && !isDeleted ? (isActive ? () => completeAllOccurrences(reminder) : undefined) : (isDeleted ? () => handlePermanentDelete(reminder) : undefined), isSelectionMode: selectionMode, children: _jsx(TouchableOpacity, { activeOpacity: 0.85, onPress: () => handleCardPress(reminder), onLongPress: () => handleLongPress(reminder.id, listType), delayLongPress: 200, style: [
                    styles.reminderCard,
                    isSelected && styles.selectedCard
                ], testID: `reminder-card-${reminder.id}`, children: _jsxs(View, { style: styles.reminderContent, children: [_jsxs(View, { style: styles.reminderLeft, children: [selectionMode && (_jsx(TouchableOpacity, { style: styles.selectionCheckbox, onPress: () => handleCardPress(reminder), children: isSelected ? (_jsx(CheckSquare, { size: 20, color: Material3Colors.light.primary })) : (_jsx(Square, { size: 20, color: Material3Colors.light.onSurfaceVariant })) })), _jsx(View, { style: [styles.priorityBar, { backgroundColor: PRIORITY_COLORS[reminder.priority] }] }), _jsxs(View, { style: styles.reminderInfo, children: [_jsx(Text, { style: styles.reminderTitle, children: reminder.title }), _jsxs(View, { style: styles.reminderMeta, children: [(reminder.repeatType === 'weekly' || reminder.repeatType === 'custom') && (_jsxs(_Fragment, { children: [_jsx(Clock, { size: 14, color: Material3Colors.light.onSurfaceVariant }), _jsx(Text, { style: styles.reminderTime, children: formatTime(reminder.time) }), _jsx(Text, { style: styles.metaSeparator, children: "\u2022" })] })), (reminder.repeatType !== 'none' && reminder.repeatType !== 'monthly' && reminder.repeatType !== 'yearly' && reminder.repeatType !== 'every' && reminder.repeatType !== 'daily') && (_jsx(View, { style: styles.repeatBadge, children: _jsx(Text, { style: styles.repeatBadgeText, children: formatRepeatType(reminder.repeatType, reminder.everyInterval) }) }))] }), _jsxs(View, { style: styles.reminderDetails, children: [reminder.repeatType === 'none' && !reminder.isCompleted && (_jsxs(View, { style: styles.nextOccurrenceContainer, children: [_jsx(Clock, { size: 14, color: Material3Colors.light.primary }), _jsx(Text, { style: styles.reminderNextOccurrenceLarge, children: (() => {
                                                                const [year, month, day] = reminder.date.split('-').map(Number);
                                                                const date = new Date(year, month - 1, day);
                                                                ...
                                                                return `${dateStr} at ${timeStr}`;
                                                            })() })] })), (reminder.repeatType === 'weekly' || reminder.repeatType === 'custom') && reminder.repeatDays && reminder.repeatDays.length > 0 && (_jsx(Text, { style: styles.reminderDays, children: formatDays(reminder.repeatDays) })), (reminder.repeatType === 'weekly' || reminder.repeatType === 'custom') && endsLabel && (_jsx(Text, { style: styles.reminderNextOccurrence, children: endsLabel })), reminder.repeatType === 'daily' && !reminder.isCompleted && (_jsxs(_Fragment, { children: [_jsxs(View, { style: styles.nextOccurrenceContainer, children: [_jsx(Clock, { size: 14, color: Material3Colors.light.primary }), _jsx(Text, { style: styles.reminderNextOccurrenceLarge, children: (() => {
                                                                        const hasEndCondition = reminder.untilType === 'count' || reminder.untilType === 'endsAt';
                                                                        const getNextDate = () => {
                                                                            if (reminder.snoozeUntil)
                                                                                return new Date(reminder.snoozeUntil);
                                                                        ...
                                                                        if (!nextDate && hasEndCondition) {
                                                                            const lastDate = (() => {
                                                                                if (reminder.lastTriggeredAt)
                                                                                    return new Date(reminder.lastTriggeredAt);
                                                                                if (reminder.nextReminderDate)
                                                                                    return new Date(reminder.nextReminderDate);
                                                                                const [year, month, day] = reminder.date.split('-').map(Number);
                                                                                const [hours, minutes] = reminder.time.split(':').map(Number);
                                                                        ...
                                                                        return `${dateStr} at ${timeStr}`;
                                                                    })() })] }), endsLabel && (_jsx(Text, { style: styles.reminderNextOccurrence, children: endsLabel })), _jsx(View, { style: styles.dailyDaysContainer, children: [0, 1, 2, 3, 4, 5, 6].map((day) => {
                                                                ...
                                                                return (_jsx(View, { style: [
                                                                        ...
                                                                        ], children: dayLetters[day] }) }, day));
                                                            }) })] })), (reminder.repeatType === 'monthly' || reminder.repeatType === 'yearly' || reminder.repeatType === 'every') && !reminder.isCompleted && (_jsxs(View, { style: styles.nextOccurrenceContainer, children: [_jsx(Clock, { size: 14, color: Material3Colors.light.primary }), _jsx(Text, { style: styles.reminderNextOccurrenceLarge, children: (() => {
                                                                const hasEndCondition = reminder.untilType === 'count' || reminder.untilType === 'endsAt';
                                                                const getNextDate = () => {
                                                                    if (reminder.snoozeUntil)
                                                                        return new Date(reminder.snoozeUntil);
                                                                ...
                                                                return `${dateStr} at ${timeStr}`;
                                                            })() })] })), (reminder.repeatType === 'monthly' || reminder.repeatType === 'yearly') && endsLabel && (_jsx(Text, { style: styles.reminderNextOccurrence, children: endsLabel })), reminder.repeatType === 'every' && !reminder.isCompleted && (_jsx(Text, { style: styles.reminderNextOccurrence, children: (() => {
                                                        ...
                                                        return endsLabel ? `${repeatText} · ${endsLabel}` : repeatText;
                                                    })() })), reminder.repeatType === 'every' && reminder.multiSelectEnabled && (_jsxs(_Fragment, { children: [reminder.multiSelectDays && reminder.multiSelectDays.length > 0 && (_jsx(View, { style: styles.dailyDaysContainer, children: [0, 1, 2, 3, 4, 5, 6].map((day) => {
                                                                const dayLetters = ['S', 'M', 'T', 'W', 'T', 'F', 'S'];
                                                                ...
                                                                return (_jsx(View, { style: [
                                                            ...
                                                            }) })), reminder.multiSelectDates && reminder.multiSelectDates.length > 0 && (_jsx(TouchableOpacity, { onPress: () => openMultiDatesPopup(reminder.multiSelectDates, reminder.title), children: _jsx(Text, { style: styles.selectedDatesText, children: (() => {
                                                                    ...
                                                                    return (_jsxs(_Fragment, { children: [displayDates, ' ', _jsxs(Text, { style: styles.selectedDatesMoreText, children: ["+", dates.length - 3] })] }));
                                                                })() }) }))] })), (reminder.repeatType === 'none' || reminder.repeatType === 'monthly' || reminder.repeatType === 'yearly' || reminder.repeatType === 'every' || reminder.repeatType === 'daily') && (_jsxs(View, { style: styles.repeatBadgeContainer, children: [_jsx(View, { style: [styles.repeatBadge, styles.repeatBadgeBottom], children: _jsx(Text, { style: styles.repeatBadgeText, children: formatRepeatType(reminder.repeatType, reminder.everyInterval) }) }), reminder.repeatType === 'every' && reminder.multiSelectEnabled && (_jsx(View, { style: [styles.repeatBadge, styles.multiBadge], children: _jsx(Text, { style: styles.repeatBadgeText, children: "Multi" }) })), reminder.snoozeUntil && isActive && !reminder.isCompleted && (_jsxs(View, { style: styles.snoozedBadgeInline, children: [_jsx(Clock, { size: 10, color: Material3Colors.light.tertiary }), _jsx(Text, { style: styles.snoozedTextInline, children: "Snoozed" })] })), reminder.isPaused && isActive && !reminder.isCompleted && (_jsxs(View, { style: styles.pausedBadgeInline, children: [_jsx(PauseCircle, { size: 10, color: Material3Colors.light.tertiary }), _jsx(Text, { style: styles.pausedTextInline, children: "Paused" })] })), (reminder.repeatType === 'daily' || reminder.repeatType === 'monthly' || reminder.repeatType === 'yearly' || reminder.repeatType === 'every') && (_jsx(Repeat, { size: 12, color: Material3Colors.light.primary, style: { alignSelf: 'center' } }))] })), reminder.snoozeUntil && (_jsxs(Text, { style: styles.snoozeUntilText, children: ["Snoozed until: ", formatDate(reminder.snoozeUntil), " at ", formatTime(new Date(reminder.snoozeUntil).toTimeString().slice(0, 5))] })), reminder.repeatType === 'daily' && reminder.pauseUntilDate && (_jsxs(Text, { style: styles.snoozeUntilText, children: ["Paused until ", formatDate(reminder.pauseUntilDate)] })), (reminder.repeatType === 'weekly' || reminder.repeatType === 'custom') && !reminder.snoozeUntil && !reminder.isCompleted && (_jsx(Text, { style: styles.nextReminderText, children: (() => {
                                                        const hasEndCondition = reminder.untilType === 'count' || reminder.untilType === 'endsAt';
                                                        const getNextDate = () => {
                                                            if (reminder.nextReminderDate)
                                                                return new Date(reminder.nextReminderDate);
                                                        ...
                                                        if (!nextDate)
                                                            return 'Next: Calculating...';
                                                        return `Next: ${formatDate(nextDate.toISOString())}`;
                                                    })() }))] })] })] }), isActive && !isDeleted && (_jsxs(View, { style: styles.reminderRight, children: [reminder.repeatType !== 'none' && (reminder.isPaused ? (_jsx(TouchableOpacity, { style: styles.resumeButton, onPress: (e) => {
                                        e.stopPropagation();
                                        pauseReminder(reminder);
                                    }, testID: `resume-button-${reminder.id}`, children: _jsx(PlayCircle, { size: 20, color: Material3Colors.light.primary }) })) : (_jsx(TouchableOpacity, { style: styles.pauseButton, onPress: (e) => {
                                        e.stopPropagation();
                                        // Pause the reminder
                                        pauseReminder(reminder);
                                    ...
                                    }, delayLongPress: 300, testID: `pause-button-${reminder.id}`, children: _jsx(PauseCircle, { size: 20, color: "#E57373" }) }))), _jsx(TouchableOpacity, { style: styles.doneButton, onPress: (e) => {
                                        e.stopPropagation();
                                        completeReminder(reminder);
                                    }, testID: `done-button-${reminder.id}`, children: _jsx(CheckCircle, { size: 20, color: "white" }) })] }))] }) }) }));
    }, (prevProps, nextProps) => {
        // ID change always means different card
        if (prevProps.reminder.id !== nextProps.reminder.id)
            return false;
        if (prevProps.listType !== nextProps.listType)
            return false;
    ...
    if (isLoading) {
        return (_jsx(SafeAreaView, { style: styles.container, edges: ['left', 'right', 'bottom'], children: _jsxs(View, { style: styles.loadingContainer, children: [_jsx(Clock, { size: 48, color: Material3Colors.light.primary }), _jsx(Text, { style: styles.loadingText, children: "Loading reminders..." })] }) }));
    }
    return (_jsxs(_Fragment, { children: [_jsx(KeyboardAvoidingView, { style: { flex: 1 }, enabled: false, children: _jsxs(SafeAreaView, { style: styles.container, edges: ['left', 'right', 'bottom'], children: [_jsxs(View, { style: [styles.header, { paddingTop: insets.top + 12 }], children: [_jsx(Text, { style: styles.title, children: "DoMinder" }), _jsxs(View, { style: styles.headerActions, children: [Platform.OS === 'web' && (_jsx(TouchableOpacity, { style: styles.settingsButton, onPress: openOnboardingPreview, children: _jsx(HelpCircle, { size: 20, color: Material3Colors.light.onSurfaceVariant }) })), _jsx(TouchableOpacity, { style: styles.settingsButton, onPress: () => router.push('/settings'), children: _jsx(Settings, { size: 20, color: Material3Colors.light.onSurfaceVariant }) })] })] }), _jsxs(View, { style: styles.metroTabContainer, children: [_jsxs(View, { style: styles.metroTabContent, children: [_jsxs(TouchableOpacity, { style: styles.metroTab, onPress: () => scrollToTab('active'), activeOpacity: 0.7, onLayout: (e) => handleTabLayout('active', e), children: [_jsx(Text, { style: [styles.metroTabText, activeTab === 'active' && styles.metroTabTextActive], children: "active reminders" }), _jsx(Text, { style: [styles.metroTabCount, activeTab === 'active' && styles.metroTabCountActive], children: activeReminders.length })] }), _jsxs(TouchableOpacity, { style: styles.metroTab, onPress: () => scrollToTab('completed'), activeOpacity: 0.7, onLayout: (e) => handleTabLayout('completed', e), children: [_jsx(Text, { style: [styles.metroTabText, activeTab === 'completed' && styles.metroTabTextActive], children: "completed" }), _jsx(Text, { style: [styles.metroTabCount, activeTab === 'completed' && styles.metroTabCountActive], children: completedReminders.length })] }), _jsxs(TouchableOpacity, { style: styles.metroTab, onPress: () => scrollToTab('deleted'), activeOpacity: 0.7, onLayout: (e) => handleTabLayout('deleted', e), children: [_jsx(Text, { style: [styles.metroTabText, activeTab === 'deleted' && styles.metroTabTextActive], children: "deleted" }), _jsx(Text, { style: [styles.metroTabCount, activeTab === 'deleted' && styles.metroTabCountActive], children: deletedReminders.length })] }), _jsx(Animated.View, { style: {
                                                position: 'absolute',
                                                bottom: 0,
                                                left: indicatorX,
                                            ...
                                            } })] }), _jsx(View, { style: styles.metroTabDivider })] }), isSelectionMode && (_jsxs(View, { style: styles.selectionBar, children: [_jsx(TouchableOpacity, { style: styles.closeSelectionButton, onPress: exitSelectionMode, children: _jsx(X, { size: 20, color: Material3Colors.light.onSurface }) }), _jsxs(Text, { style: styles.selectionCount, children: [selectedReminders.size, " selected"] }), _jsx(View, { style: styles.selectionActions, children: (() => {
                                        const scope = selectionTab ?? activeTab;
                                        return (_jsxs(_Fragment, { children: [scope === 'active' && (_jsx(TouchableOpacity, { style: styles.selectionActionButton, onPress: markAllAsDone, disabled: selectedReminders.size === 0, testID: "bulk-done", children: _jsx(CheckCircle, { size: 16, color: selectedReminders.size === 0 ? Material3Colors.light.outline : Material3Colors.light.primary }) })), _jsx(TouchableOpacity, { style: styles.selectionActionButton, onPress: deleteAll, disabled: selectedReminders.size === 0, testID: "bulk-delete", children: _jsx(Trash2, { size: 16, color: selectedReminders.size === 0 ? Material3Colors.light.outline : Material3Colors.light.error }) }), scope === 'active' && (_jsx(TouchableOpacity, { style: styles.selectionActionButton, onPress: pauseAll, disabled: selectedReminders.size === 0, testID: "bulk-pause", children: _jsx(PauseCircle, { size: 16, color: selectedReminders.size === 0 ? Material3Colors.light.outline : Material3Colors.light.onSurfaceVariant }) })), _jsx(TouchableOpacity, { style: styles.selectionActionButton, onPress: selectAll, testID: "bulk-select-all", children: _jsx(CheckSquare, { size: 16, color: Material3Colors.light.primary }) })] }));
                                    })() })] })), _jsx(FlashList, { ref: contentScrollRef, data: currentList, renderItem: ({ item }) => (_jsx(ReminderCard, { reminder: item, listType: activeTab, isSelected: selectedReminders.has(item.id), isSelectionMode: isSelectionMode })), extraData: selectionTimestamp, estimatedItemSize: 120, keyExtractor: (item) => item.id.toString(), showsVerticalScrollIndicator: false, scrollEnabled: !showCreatePopup && !showOnboarding, ItemSeparatorComponent: () => null, contentContainerStyle: {
                                paddingBottom: 100,
                                paddingTop: 4,
                                paddingHorizontal: 0,
                            }, drawDistance: Platform.OS === 'android' ? 500 : 250, removeClippedSubviews: false, overrideItemLayout: (layout, item) => {
                                if (Platform.OS === 'android') {
                                    layout.size = layout.size || 120;
                                }
                            }, ListEmptyComponent: _jsx(View, { style: styles.emptyState, children: activeTab === 'active' ? (_jsxs(_Fragment, { children: [_jsx(Clock, { size: 64, color: Material3Colors.light.outline }), _jsx(Text, { style: styles.emptyTitle, children: "No Active Reminders" }), _jsx(Text, { style: styles.emptyDescription, children: "Tap (+) button to create a reminder" })] })) : activeTab === 'completed' ? (_jsxs(_Fragment, { children: [_jsx(CheckCircle, { size: 64, color: Material3Colors.light.outline }), _jsx(Text, { style: styles.emptyTitle, children: "No Completed Reminders" }), _jsx(Text, { style: styles.emptyDescription, children: "Completed reminders will appear here" })] })) : (_jsxs(_Fragment, { children: [_jsx(Trash2, { size: 64, color: Material3Colors.light.outline }), _jsx(Text, { style: styles.emptyTitle, children: "No Deleted Reminders" }), _jsx(Text, { style: styles.emptyDescription, children: "Deleted reminders will appear here" })] })) }), bounces: true, bouncesZoom: false, alwaysBounceVertical: true, overScrollMode: "always" }), _jsx(Modal, { visible: historyPopupVisible, transparent: true, animationType: "fade", onRequestClose: () => setHistoryPopupVisible(false), children: _jsx(Pressable, { style: styles.historyPopupOverlay, onPress: () => setHistoryPopupVisible(false), children: _jsxs(View, { style: styles.historyPopupContent, onStartShouldSetResponder: () => true, children: [_jsx(Text, { style: styles.historyPopupTitle, children: "History" }), _jsx(FlatList, { data: historyPopupData, keyExtractor: (item) => item, renderItem: ({ item }) => {
                                                const d = new Date(item);
                                                return (_jsx(View, { style: styles.historyPopupItem, children: _jsxs(Text, { style: styles.historyPopupItemText, children: [d.toLocaleDateString(), " at ", formatTime(d.toTimeString().slice(0, 5))] }) }));
                                            }, style: styles.historyPopupList, showsVerticalScrollIndicator: true }), _jsx(TouchableOpacity, { style: styles.closeHistoryButton, onPress: () => setHistoryPopupVisible(false), children: _jsx(Text, { style: styles.closeHistoryButtonText, children: "Close" }) })] }) }) }), _jsx(Modal, { visible: multiDatesPopupVisible, transparent: true, animationType: "fade", onRequestClose: () => setMultiDatesPopupVisible(false), children: _jsx(Pressable, { style: styles.historyPopupOverlay, onPress: () => setMultiDatesPopupVisible(false), children: _jsxs(View, { style: styles.historyPopupContent, onStartShouldSetResponder: () => true, children: [_jsx(Text, { style: styles.historyPopupTitle, children: "Selected Dates" }), _jsx(Text, { style: [styles.historyPopupTitle, { fontSize: 14, fontWeight: 'normal', marginBottom: 16 }], children: multiDatesPopupTitle }), _jsx(FlatList, { data: multiDatesPopupData, keyExtractor: (item) => item, renderItem: ({ item }) => {
                                                ...
                                                return (_jsx(View, { style: styles.historyPopupItem, children: _jsx(Text, { style: styles.historyPopupItemText, children: dateStr }) }));
                                            }, style: styles.historyPopupList, showsVerticalScrollIndicator: true }), _jsx(TouchableOpacity, { style: styles.closeHistoryButton, onPress: () => setMultiDatesPopupVisible(false), children: _jsx(Text, { style: styles.closeHistoryButtonText, children: "Close" }) })] }) }) }), _jsx(CreateReminderPopup, { visible: showCreatePopup, onClose: () => setShowCreatePopup(false), title: title, onTitleChange: setTitle, selectedTime: selectedTime, isAM: isAM, untilTime: untilTime, untilIsAM: untilIsAM, priority: priority, onPriorityChange: setPriority, repeatType: repeatType, onRepeatTypeChange: setRepeatType, repeatDays: repeatDays, onRepeatDaysChange: setRepeatDays, everyValue: everyValue, everyUnit: everyUnit, onEveryChange: (value, unit) => {
                                setEveryValue(value);
                                setEveryUnit(unit);
                            }, onTimeSelect: () => { setTimeSelectorContext('start'); setShowTimeSelector(true); }, onTimeChange: (time, ampm) => {
                                setSelectedTime(time);
                                setIsAM(ampm);
                            }, onUntilTimeChange: (time, ampm) => {
                                setUntilTime(time);
                                setUntilIsAM(ampm);
                            }, showTimeSelector: showTimeSelector, onCloseTimeSelector: () => setShowTimeSelector(false), timeSelectorContext: timeSelectorContext, 
                            // Until props
                            untilType: untilType, untilDate: untilDate, untilCount: untilCount, onUntilTypeChange: (type) => {
                                setUntilType(type);
                            ...
                            }, onUntilDateChange: setUntilDate, onUntilCountChange: (count) => setUntilCount(Math.min(999, Math.max(1, count))), onOpenUntilTime: () => { setTimeSelectorContext('until'); setShowTimeSelector(true); }, 
                            // Multi-select props
                            multiSelectEnabled: multiSelectEnabled, onMultiSelectEnabledChange: setMultiSelectEnabled, multiSelectDates: multiSelectDates, onMultiSelectDatesChange: (dates) => {
                                const sorted = [...dates].sort((a, b) => a.localeCompare(b));
                                setMultiSelectDates(sorted);
                            }, multiSelectDays: multiSelectDays, onMultiSelectDaysChange: setMultiSelectDays, onConfirm: () => {
                                if (!title.trim()) {
                                ...
                                if (editingReminder) {
                                    ...
                                    if (editingReminder.isCompleted) {
                                        ...
                                        if (repeatType === 'daily') {
                                            ...
                                            if (todayTime <= now) {
                                                Alert.alert('Past Time Selected', 'The selected time has already passed for today. This reminder will be active starting tomorrow.', [{ text: 'OK' }]);
                                            }
                                        }
                                    }
                                    const updated = {
                                    ...
                                    updateReminder.mutate(updated, {
                                        onSuccess: () => {
                                            ...
                                            const mappedPriority = defaultPriority === 'standard' ? 'medium' :
                            ...
                            }, isLoading: addReminder.isPending || updateReminder.isPending, selectedDate: selectedDate, onDateChange: setSelectedDate, mode: editingReminder ? 'edit' : 'create' })] }) }), !isSelectionMode && (_jsx(View, { style: styles.bottomContainer, children: _jsx(TouchableOpacity, { style: styles.createAlarmButton, onPress: () => {
                        setEditingReminder(null);
                        setTitle('');
                        const defaultPriority = settings?.defaultPriority ?? 'standard';
                        const mappedPriority = defaultPriority === 'standard' ? 'medium' : defaultPriority === 'silent' ? 'low' : 'high';
                        setPriority(mappedPriority);
                        setRepeatType(settings?.defaultReminderMode ?? 'none');
                        setRepeatDays([]);
                        setEveryValue(1);
                        ...
                        setMultiSelectDays([]);
                        setShowCreatePopup(true);
                    }, testID: "fab-create-reminder", children: _jsx(Plus, { size: 32, color: "white" }) }) })), _jsx(CalendarModal, { visible: pauseUntilCalendarVisible, onClose: () => {
                    setPauseUntilCalendarVisible(false);
                    setPauseUntilReminder(null);
                }, selectedDate: selectedDate, onSelectDate: handlePauseUntilDate, disablePast: true, hideYear: false, title: "Pause Until" }), _jsx(OnboardingFlow, { visible: showOnboarding, onSkip: completeOnboarding, onComplete: completeOnboarding })] }));
}
function CreateReminderPopup({ visible, onClose, title, onTitleChange, selectedTime, isAM, untilTime, untilIsAM, priority, onPriorityChange, repeatType, onRepeatTypeChange, repeatDays, onRepeatDaysChange, onTimeSelect, onTimeChange, onUntilTimeChange, showTimeSelector, onCloseTimeSelector, onConfirm, isLoading, selectedDate, onDateChange, mode, everyValue, everyUnit, onEveryChange, timeSelectorContext, 
// Until props
untilType, untilDate, untilCount, onUntilTypeChange, onUntilDateChange, onUntilCountChange, onOpenUntilTime, multiSelectEnabled, onMultiSelectEnabledChange, multiSelectDates, onMultiSelectDatesChange, multiSelectDays, onMultiSelectDaysChange, }) {
    ...
    if (!visible)
        return null;
    return (_jsxs(Modal, { visible: visible, transparent: true, animationType: "fade", onRequestClose: onClose, presentationStyle: "overFullScreen", statusBarTranslucent: true, children: [_jsx(KeyboardAvoidingView, { behavior: Platform.OS === 'ios' ? 'padding' : undefined, style: { flex: 1 }, children: _jsx(Pressable, { style: createPopupStyles.overlay, onPress: () => {
                        RNKeyboard.dismiss();
                        onClose();
                    }, children: _jsxs(Pressable, { onPress: (e) => e.stopPropagation(), style: [
                            scaledStyles.popup,
                            {
                                opacity: isReady ? 1 : 0,
                                maxHeight: '85%',
                        ...
                        ], children: [_jsx(ScrollView, { showsVerticalScrollIndicator: false, contentContainerStyle: { paddingBottom: 4, flexGrow: isLandscape ? 1 : 0 }, style: { flexShrink: 1 }, keyboardDismissMode: "none", keyboardShouldPersistTaps: "always", scrollEnabled: true, children: _jsx(Pressable, { style: { flex: 1 }, onPress: () => {
                                        // Close any open dropdowns when tapping blank space
                                        if (window.__closeCustomizePanelDropdowns) {
                                            window.__closeCustomizePanelDropdowns();
                                        }
                                    }, children: _jsx(View, { style: createPopupStyles.mainContent, onStartShouldSetResponder: () => true, onResponderRelease: () => {
                                            // Close dropdowns when tapping in this area
                                            if (window.__closeCustomizePanelDropdowns) {
                                                window.__closeCustomizePanelDropdowns();
                                            }
                                        }, children: _jsxs(Pressable, { onPress: (e) => {
                                                e.stopPropagation();
                                                // Also close dropdowns when tapping inside this area
                                                if (window.__closeCustomizePanelDropdowns) {
                                                    window.__closeCustomizePanelDropdowns();
                                                }
                                            }, children: [_jsx(View, { style: scaledStyles.section, children: _jsx(TextInput, { ref: titleInputRef, style: scaledStyles.titleInput, placeholder: "Enter reminder", placeholderTextColor: "#9CA3AF", value: title, onChangeText: onTitleChange, onFocus: () => {
                                                        ...
                                                        }, returnKeyType: "done", blurOnSubmit: true, maxLength: 100, testID: "title-input" }) }), _jsx(View, { style: [scaledStyles.customizeContent, repeatType === 'every' && { marginBottom: 2 * scaleFactor }], children: _jsx(CustomizePanel, { repeatType: repeatType, repeatDays: repeatDays, onRepeatTypeChange: onRepeatTypeChange, onRepeatDaysChange: onRepeatDaysChange, selectedDate: selectedDate, onDateChange: (date) => {
                                                            onDateChange(date);
                                                            // Don't dismiss keyboard on date change - only when opening time picker
                                                        }, onOpenTime: () => { onTimeSelect(); }, displayTime: `${formatTime(selectedTime, isAM)}`, everyValue: everyValue, everyUnit: everyUnit, onEveryChange: onEveryChange, 
                                                        // Until props
                                                        untilTime: untilTime, untilIsAM: untilIsAM, untilType: untilType, untilDate: untilDate, untilCount: untilCount, onUntilTypeChange: onUntilTypeChange, onUntilDateChange: onUntilDateChange, onUntilCountChange: onUntilCountChange, onOpenUntilTime: onOpenUntilTime, onDropdownStateChange: () => { }, scaleFactor: scaleFactor, isLandscape: isLandscape, 
                                                        // Multi-select
                                                        multiSelectEnabled: multiSelectEnabled, onMultiSelectEnabledChange: onMultiSelectEnabledChange, multiSelectDates: multiSelectDates, onMultiSelectDatesChange: onMultiSelectDatesChange, multiSelectDays: multiSelectDays, onMultiSelectDaysChange: onMultiSelectDaysChange, onSetTime: () => {
                                                            ...
                                                            onTimeSelect();
                                                        } }) }), _jsx(View, { style: scaledStyles.section, children: _jsx(PrioritySelector, { priority: priority, onPriorityChange: onPriorityChange }) })] }) }) }) }), _jsxs(View, { style: scaledStyles.buttonContainer, children: [_jsx(TouchableOpacity, { style: scaledStyles.cancelButton, onPress: onClose, testID: "cancel-create", children: _jsx(Text, { style: scaledStyles.cancelButtonText, children: "Cancel" }) }), _jsx(TouchableOpacity, { style: [scaledStyles.createButton, isLoading && createPopupStyles.createButtonDisabled], onPress: onConfirm, disabled: isLoading, testID: "confirm-create", children: _jsx(Text, { style: scaledStyles.createButtonText, children: isLoading ? (mode === 'edit' ? 'Rescheduling...' : 'Creating...') : (mode === 'edit' ? 'Reschedule' : 'Create') }) })] })] }) }) }), _jsx(TimeSelector, { visible: showTimeSelector, selectedTime: timeSelectorContext === 'until' ? untilTime : selectedTime, isAM: timeSelectorContext === 'until' ? untilIsAM : isAM, onTimeChange: (t, am) => {
                    if (timeSelectorContext === 'until')
                        onUntilTimeChange(t, am);
                    else
                        onTimeChange(t, am);
                }, onClose: onCloseTimeSelector, selectedDate: timeSelectorContext === 'until' ? (untilDate || selectedDate) : selectedDate, repeatType: repeatType, onPastTimeError: (msg) => showToast(msg ?? 'Please select a future time') })] }));
}
...
function TimeSelector({ visible, selectedTime, isAM, onTimeChange, onClose, selectedDate, repeatType, onPastTimeError }) {
    ...
    useEffect(() => {
        if (!visible || isDragging.current)
            return; // Don't update if dragging
        const hourStep = 360 / 12;
    ...
    const [discSize, setDiscSize] = useState(220);
    ...
    const decayAnimation = useRef(null);
    const snapAnimation = useRef(null);
    const autoSwitchTimeout = useRef(null);
    const stoppedByFriction = useRef(false);
    const lastValueUpdate = useRef(0);
    ...
    const measureCenter = () => {
        try {
            discRef.current?.measureInWindow?.((x, y, w, h) => {
    ...
    const panResponder = PanResponder.create({
        ...
        onPanResponderMove: (evt, gestureState) => {
            ...
            if (Math.abs(delta) < DEADBAND_DEG)
                return;
    ...
    if (!visible)
        return null;
    return (_jsx(Modal, { visible: visible, transparent: true, animationType: "fade", onRequestClose: onClose, presentationStyle: "overFullScreen", statusBarTranslucent: true, onShow: () => setIsReady(true), children: _jsx(TouchableOpacity, { style: timeSelectorStyles.overlay, activeOpacity: 1, onPress: () => {
                if (showManualEntry) {
                    ...
                    isLandscape && timeSelectorStyles.containerLandscape,
                    {
                        ...
                        ...(Platform.OS === 'android' && {
                            elevation: isReady ? 10 : 0
                ...
                ], activeOpacity: 1, onPress: (e) => e.stopPropagation(), children: isLandscape ? (_jsxs(View, { style: timeSelectorStyles.landscapeRow, children: [_jsxs(View, { style: timeSelectorStyles.sidePanel, children: [_jsx(View, { style: timeSelectorStyles.timeDisplay, children: showManualEntry ? (_jsx(TouchableOpacity, { style: timeSelectorStyles.manualEntryContainer, activeOpacity: 1, onPress: () => {
                                            const timeRegex = /^([0-1]?[0-9]|2[0-3]):([0-5][0-9])$/;
                                            if (timeRegex.test(manualTimeInput)) {
                                                const [inputHour, inputMinute] = manualTimeInput.split(':').map(Number);
                                                let hour12 = inputHour;
                                        ...
                                        }, children: _jsx(TextInput, { style: timeSelectorStyles.manualTimeInput, value: manualTimeInput, onChangeText: (text) => {
                                            ...
                                            }, placeholder: "HH:MM", placeholderTextColor: "#9CA3AF", keyboardType: "numeric", maxLength: 5, autoFocus: true, selectTextOnFocus: true, testID: "manual-time-input" }) })) : (_jsxs(_Fragment, { children: [_jsx(TouchableOpacity, { style: [
                                                    timeSelectorStyles.timeSection,
                                                    activeSection === 'hour' && timeSelectorStyles.activeSectionLeft
                                                ], onPress: () => handleSectionPress('hour'), children: _jsx(Text, { style: [
                                                        timeSelectorStyles.timeSectionText,
                                                        activeSection === 'hour' && timeSelectorStyles.activeTimeSectionText
                                                    ], children: currentHour.toString().padStart(2, '0') }) }), _jsx(Text, { style: timeSelectorStyles.timeSeparator, children: ":" }), _jsx(TouchableOpacity, { style: [
                                                    timeSelectorStyles.timeSection,
                                                    activeSection === 'minute' && timeSelectorStyles.activeSectionRight
                                                ], onPress: () => handleSectionPress('minute'), children: _jsx(Text, { style: [
                                                        timeSelectorStyles.timeSectionText,
                                                        activeSection === 'minute' && timeSelectorStyles.activeTimeSectionText
                                                    ], children: currentMinute.toString().padStart(2, '0') }) })] })) }), _jsxs(View, { style: timeSelectorStyles.ampmContainer, children: [_jsx(TouchableOpacity, { style: [
                                                timeSelectorStyles.ampmButton,
                                                currentAMPM && timeSelectorStyles.selectedAMPM
                                            ], onPress: () => setCurrentAMPM(true), children: _jsx(Text, { style: [
                                                    timeSelectorStyles.ampmText,
                                                    currentAMPM && timeSelectorStyles.selectedAMPMText
                                                ], children: "AM" }) }), _jsx(TouchableOpacity, { style: [
                                                timeSelectorStyles.ampmButton,
                                                !currentAMPM && timeSelectorStyles.selectedAMPM
                                            ], onPress: () => setCurrentAMPM(false), children: _jsx(Text, { style: [
                                                    timeSelectorStyles.ampmText,
                                                    !currentAMPM && timeSelectorStyles.selectedAMPMText
                                                ], children: "PM" }) })] }), _jsxs(View, { style: timeSelectorStyles.buttonContainer, children: [_jsx(TouchableOpacity, { style: timeSelectorStyles.keyboardButton, onPress: () => {
                                                setShowManualEntry(true);
                                                setManualTimeInput('');
                                            }, testID: "keyboard-button", children: _jsx(Keyboard, { size: 16, color: "#6B7280" }) }), _jsxs(View, { style: timeSelectorStyles.rightButtons, children: [_jsx(TouchableOpacity, { style: timeSelectorStyles.cancelButton, onPress: onClose, children: _jsx(Text, { style: timeSelectorStyles.cancelButtonText, children: "Cancel" }) }), _jsx(TouchableOpacity, { style: timeSelectorStyles.confirmButton, onPress: handleConfirm, children: _jsx(Text, { style: timeSelectorStyles.confirmButtonText, children: "Set" }) })] })] })] }), _jsx(View, { style: timeSelectorStyles.discPanel, children: _jsx(View, { style: timeSelectorStyles.discContainer, children: _jsx(View, { ref: discRef, collapsable: false, onLayout: (e) => {
                                        const w = e.nativeEvent.layout?.width ?? 220;
                                        setDiscSize(typeof w === 'number' ? w : 220);
                                    }, style: timeSelectorStyles.discBackground, ...panResponder.panHandlers, testID: "time-disc", children: _jsxs(View, { style: [timeSelectorStyles.handContainer, { transform: [{ rotate: `${rotation}deg` }] }], children: [renderTickMarks(), _jsx(View, { style: timeSelectorStyles.discIndicator })] }) }) }) })] })) : (_jsxs(_Fragment, { children: [_jsx(View, { style: timeSelectorStyles.timeDisplay, children: showManualEntry ? (_jsx(TouchableOpacity, { style: timeSelectorStyles.manualEntryContainer, activeOpacity: 1, onPress: () => {
                                    const timeRegex = /^([0-1]?[0-9]|2[0-3]):([0-5][0-9])$/;
                                    if (timeRegex.test(manualTimeInput)) {
                                        const [inputHour, inputMinute] = manualTimeInput.split(':').map(Number);
                                        let hour12 = inputHour;
                                        ...
                                        if (timeRegex.test(manualTimeInput)) {
                                            ...
                                            activeSection === 'minute' && timeSelectorStyles.activeSectionRight
                                                ...
                                                timeSelectorStyles.timeSectionText,
                                                activeSection === 'minute' && timeSelectorStyles.activeTimeSectionText
                                            ], children: currentMinute.toString().padStart(2, '0') }) })] })) }), _jsx(View, { style: timeSelectorStyles.discContainer, children: _jsx(View, { ref: discRef, collapsable: false, onLayout: (e) => {
                                    const w = e.nativeEvent.layout?.width ?? 220;
                                    setDiscSize(typeof w === 'number' ? w : 220);
                                }, style: timeSelectorStyles.discBackground, ...panResponder.panHandlers, testID: "time-disc", children: _jsxs(View, { style: [timeSelectorStyles.handContainer, { transform: [{ rotate: `${rotation}deg` }] }], children: [renderTickMarks(), _jsx(View, { style: timeSelectorStyles.discIndicator })] }) }) }), _jsxs(View, { style: timeSelectorStyles.ampmContainer, children: [_jsx(TouchableOpacity, { style: [
                                        timeSelectorStyles.ampmButton,
                                        currentAMPM && timeSelectorStyles.selectedAMPM
                                    ], onPress: () => setCurrentAMPM(true), children: _jsx(Text, { style: [
                                            timeSelectorStyles.ampmText,
