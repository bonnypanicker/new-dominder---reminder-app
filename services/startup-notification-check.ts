...
export async function checkAndTriggerPendingNotifications() {
  ...
  try {
    ...
    for (const reminder of allReminders) {
      ...
      // SKIP if there is a pending snooze (handled by Shadow Snooze native alarm)
      if (nativeStates[reminder.id]?.hasPendingSnooze) {
        console.log(`[StartupCheck] Skipping overdue check for ${reminder.id} - has PENDING SNOOZE`);
        continue;
      }
