...
function resolveMonthlyDay(year, monthIndex, desiredDay) {
    const lastDay = endOfMonth(year, monthIndex);
    return Math.min(desiredDay, lastDay);
}
function nextMonthlyOccurrenceFrom(now, desiredDay, hh, mm) {
...
export function calculateNextReminderDate(reminder, fromDate = new Date()) {
