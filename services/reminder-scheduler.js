...
export async function rescheduleReminderById(reminderId, minutes) {
    ...
    if (reminder.repeatType !== 'none') {
        ...
        if (AlarmModule?.storeReminderMetadata) {
            try {
                ...
                await AlarmModule.storeReminderMetadata(shadowId, 'none', // Repeat 'none' for snooze instance
                1, 'minutes', 'forever', 0, '', '', // until
                0, // occurrence
                sDate, sTime, `Snoozed: ${reminder.title}`, priority, false, '[]', '[]', '', false // multiselect
                );
...
export async function markReminderDone(reminderId, shouldIncrementOccurrence = true, triggerTimeMs) {
    ...
    const getHistoryItem = async () => {
        ...
        return allReminders.find((r) => r.id === historyId);
    };
    if (reminder.repeatType === 'none') {
        await notificationService.cancelAllNotificationsForReminder(reminderId);
    }
    ...
    if (reminder.snoozeUntil && reminder.repeatType === 'none') {
        ...
        if (!alreadyInHistory) {
            if (existingHistory) {
                ...
                await updateReminder(updatedHistory);
