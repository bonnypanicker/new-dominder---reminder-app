...
export const [ReminderEngineProvider, useReminderEngine] = createContextHook(() => {
    const { data: reminders = [] } = useReminders();
    const updateReminder = useUpdateReminder();
    const [lastTick, setLastTick] = React.useState(Date.now());
    const updateReminderRef = useRef(updateReminder);
    const processedReminders = useRef(new Map());
    const schedulingInProgress = useRef(new Set());
    ...
    useEffect(() => {
        ...
        for (const reminder of reminders) {
            ...
            let reminderToSchedule = reminder;
