import notifee, { AndroidCategory, AndroidImportance, AndroidStyle, TriggerType, AuthorizationStatus, AndroidNotificationSetting, AlarmType, } from '@notifee/react-native';
import { NativeModules, Platform } from 'react-native';
const AlarmModule = Platform.OS === 'android' ? NativeModules?.AlarmModule ?? null : null;
...
function formatSmartDateTime(when) {
...
function formatRepeatType(repeatType, everyInterval) {
    switch (repeatType) {
...
function bodyWithTime(desc, when) {
...
const scheduledTimestamps = new Map(); // timestamp -> array of reminder IDs
function reminderToTimestamp(reminder) {
...
function applySequentialDelay(baseTimestamp, reminderId) {
...
export function createNotificationConfig(reminder, when) {
...
export async function scheduleReminderByModel(reminder) {
        ...
        if (!canUseNative) {
            ...
            try {
                ...
                if (AlarmModule?.storeReminderMetadata) {
                    await AlarmModule.storeReminderMetadata(reminder.id, reminder.repeatType || 'none', reminder.everyInterval?.value || 1, reminder.everyInterval?.unit || 'minutes', reminder.untilType || 'forever', reminder.untilCount || 0, reminder.untilDate || '', reminder.untilTime || '', reminder.occurrenceCount || 0, reminder.date || '', reminder.time || '', reminder.title, reminder.priority, reminder.multiSelectEnabled ?? false, JSON.stringify(reminder.multiSelectDates ?? []), JSON.stringify(reminder.multiSelectDays ?? []), reminder.windowEndTime ?? '', reminder.windowEndIsAM ?? false);
...
export async function cancelAllNotificationsForReminder(reminderId) {
    try {
...
export async function displayInfoNotification(title, body) {
    try {
