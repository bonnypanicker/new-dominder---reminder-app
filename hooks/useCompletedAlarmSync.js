...
export function useCompletedAlarmSync() {
    ...
    const syncCompletedAlarms = async () => {
        ...
        try {
                        ...
                        for (const [reminderId, state] of stateEntries) {
                            ...
                            if (nativeState.actualTriggerCount > 0) {
                                ...
                                if (reminder && (reminder.occurrenceCount || 0) < nativeState.actualTriggerCount) {
                                    console.log('[AlarmSync] Syncing occurrenceCount from native:', reminderId, 'JS:', reminder.occurrenceCount, '-> Native:', nativeState.actualTriggerCount);
            ...
            for (const [reminderId, timestamp] of completedEntries) {
                ...
                const triggerTimeMs = parseInt(timestamp, 10);
            ...
            for (const [reminderId, data] of snoozedEntries) {
                ...
                try {
                    ...
                    const [timestamp, minutesStr] = data.split(':');
     ...
     */
    const syncTriggerHistoryToJS = async (reminderId, nativeState) => {
        try {
            if (!nativeState.triggerHistory)
                return;
            ...
            const existingHistory = allReminders.find((r) => r.id === historyId);
            const mainReminder = await getReminder(reminderId);
            if (!mainReminder)
                return;
            // Convert trigger times to ISO strings
            const nativeTriggerISOs = triggerTimes.map((t) => new Date(t).toISOString());
            if (existingHistory) {
                ...
                const newTimes = nativeTriggerISOs.filter((t) => !existingTimes.has(t));
