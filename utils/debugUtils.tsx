...
export class RenderTracker {
  ...
  trackRender(componentName: string, props?: any) {
    if (!DEBUG_CONFIG.ENABLE_RENDER_TRACKING) return;
    ...
    if (lastTime) {
      ...
      if (renderDuration > DEBUG_CONFIG.VERY_SLOW_RENDER_THRESHOLD) {
        console.warn(
          `${COLORS.red}[SLOW RENDER]${COLORS.reset} ${componentName} took ${renderDuration}ms to render (threshold: ${DEBUG_CONFIG.VERY_SLOW_RENDER_THRESHOLD}ms)`
        );
      } else if (renderDuration > DEBUG_CONFIG.SLOW_RENDER_THRESHOLD) {
        console.log(
          `${COLORS.yellow}[Render Warning]${COLORS.reset} ${componentName} took ${renderDuration}ms`
        );
      }
    }
    ...
    if (count > 10 && count % 10 === 0) {
      console.warn(
        `${COLORS.red}[EXCESSIVE RENDERS]${COLORS.reset} ${componentName} has rendered ${count} times!`,
    ...
    if (count % 5 === 0) {
      console.log(
        `${COLORS.cyan}[Render Count]${COLORS.reset} ${componentName}: ${count} renders`
      );
    }
  }
  ...
  logSummary() {
    ...
    this.componentRenderCounts.forEach((count, name) => {
      ...
      console.log(
        `${name}: ${count} renders, avg: ${avgTime.toFixed(2)}ms`
      );
    });
...
export class AnimationConflictDetector {
  ...
  private animationDrivers: Map<string, Map<string, boolean>> = new Map();
  ...
  registerAnimation(
    ...
    if (!DEBUG_CONFIG.DETECT_ANIMATION_CONFLICTS) return;
  ...
  unregisterAnimation(nodeId: string, animationName: string) {
    if (!DEBUG_CONFIG.DETECT_ANIMATION_CONFLICTS) return;
  ...
  private detectConflict(nodeId: string) {
    const drivers = this.animationDrivers.get(nodeId);
    if (!drivers || drivers.size <= 1) return;

    const driverTypes = new Set(Array.from(drivers.values()));
    if (driverTypes.size > 1) {
      console.error(
        `${COLORS.red}[ANIMATION CONFLICT]${COLORS.reset} Node ${nodeId} has mixed useNativeDriver animations!`,
        '\nAnimations:',
        Array.from(drivers.entries()).map(([name, isNative]) => 
          `${name}: ${isNative ? 'native' : 'JS'}`
        )
      );
    }
  }
...
export class LayoutConflictDetector {
  private layoutUpdates: Map<string, number> = new Map();
  private layoutTimestamps: Map<string, number[]> = new Map();

  trackLayout(componentId: string, layout: { width: number; height: number; x: number; y: number }) {
    if (!DEBUG_CONFIG.DETECT_LAYOUT_CONFLICTS) return;
    ...
    if (recentUpdates.length > 10) {
      console.warn(
        `${COLORS.yellow}[LAYOUT THRASHING]${COLORS.reset} ${componentId} has ${recentUpdates.length} layout updates in last second!`,
        layout
      );
    }
    ...
    if (layout.width <= 0 || layout.height <= 0) {
      console.warn(
        `${COLORS.yellow}[INVALID LAYOUT]${COLORS.reset} ${componentId} has invalid dimensions:`,
        layout
      );
    }
  }
...
export class StateMismatchDetector {
  ...
  trackState(componentId: string, state: any, label?: string) {
    if (!DEBUG_CONFIG.ENABLE_STATE_TRACKING) return;
  ...
  compareState(componentId: string, expected: any, actual: any, message?: string) {
    if (!DEBUG_CONFIG.ENABLE_STATE_TRACKING) return;

    const isMatch = JSON.stringify(expected) === JSON.stringify(actual);
    if (!isMatch) {
      console.error(
        `${COLORS.red}[STATE MISMATCH]${COLORS.reset} ${componentId}${message ? `: ${message}` : ''}`,
        '\nExpected:', expected,
        '\nActual:', actual
      );
    }
...
export class PerformanceMonitor {
  ...
  start(label: string) {
    ...
    if (duration > 100) {
      console.warn(
        `${COLORS.yellow}[SLOW OPERATION]${COLORS.reset} ${label} took ${duration}ms`
      );
    }
  ...
  logSummary() {
    ...
    Object.entries(metrics).forEach(([name, data]: [string, any]) => {
      console.log(
        `${name}: avg=${data.average.toFixed(2)}ms, min=${data.min}ms, max=${data.max}ms (${data.count} calls)`
      );
    });
    console.log(`${COLORS.blue}===========================${COLORS.reset}\n`);
  }
...
export function withRenderTracking<P extends object>(
  ...
  const WithRenderTracking = (props: P) => {
...
if (__DEV__) {
  ...
  console.log(
    `${COLORS.green}[Debug Utils] Enabled! Access via global.debugUtils${COLORS.reset}`
  );
}
